...
# CONNECTION DETAILS
# host = "localhost"
host="haproxy-pafdb1pg1532.postgresdba-stg.svc.140.tess.io"
dbname = "postgres"
user = "postgres"
password = "postgres"
connect_timeout = 5
...
if __name__ == "__main__":
   conn = create_conn()
   # Testing autoCommit  - 1/30/2023 - https://www.cybertec-postgresql.com/en/disabling-autocommit-in-postgresql-can-damage-your-health/
         ...
         try:
            ...
            if (rows[0] == False):
               ...
               if doDML:
                  ...
                  ## print ("Before INSERT"  + '\n')
                  cur.execute("INSERT INTO HATEST VALUES(CURRENT_TIMESTAMP, '" + colo + "')")
                  print ("AFTER INSERT"  + '\n')
                  if cur.rowcount == 1 :
                     ...
                     cur.execute("select tm, colo from HATEST where colo = '" + colo + "' AND tm = (select max(tm) from HATEST)")
                     rowsx = cur.fetchone()
                     print ("MAX data is " + str(rowsx[0]) + ' === COLO: ' + str(rowsx[1]) + '\n')
                  ## print ("INSERT2 INTO lag_stats_from_primary" + '\n')
