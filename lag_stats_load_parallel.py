...
def insert(member):
  ...
  if conn is not None:
    ...
    try:
      ...
      application_name = cluster + '-' + member_index

      print(" INSERT to lag_stats_from_primary for " + application_name)
      cur.execute("INSERT INTO lag_stats_from_primary (usename       , application_name , client_addr, client_hostname  , client_port, \
                      ...
                      sent_lsn      , write_lsn        , flush_lsn  , replay_lsn       , \
                      write_lag     , flush_lag        , replay_lag , \
                      sync_priority , sync_state       , reply_time , now()     from pg_stat_replication where application_name = '" + application_name + "')")
      print(" INSERT to lag_stats_from_primary for " + application_name + " -- DONE")

      print(" INSERT to lag_stats_from_secondary from STBY " + application_name)
      ...
      print(" INSERT to lag_stats_from_secondary from STBY " + application_name + " -- DONE")
